...
int execute_and_or(AstNode *and_or)
{
    const size_t num_commands = and_or->num_children;
    int status = 0;
    for (size_t i = 0; i < num_commands; i++)
    {
        AstNode *pipeline = &and_or->children[i];
        int pipeline_status = execute_pipeline(pipeline);
        if (and_or->value != NULL)
        {
            if (i == 0 && and_or->value[0] == '&')
            {
            ...
            {
                // Combine status for subsequent pipelines
                if (and_or->value[0] == '&')
                {
                    status = status && pipeline_status;
                }
                else if (and_or->value[0] == '|')
                {
                    status = status || pipeline_status;
                }
            }
